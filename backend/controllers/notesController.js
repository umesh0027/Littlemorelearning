const NoteCategory = require("../models/Note");
...
exports.getCategoryById = async (req, res) => {
    ...
    try {
      const category = await NoteCategory.findById(categoryId); // Find category in the database by ID
      if (!category) {
...
exports.updateCategory = async (req, res) => {
  try {
    ...
    const updatedCategory = await NoteCategory.findByIdAndUpdate(
      categoryId,
      { name, description },
...
exports.deleteCategory = async (req, res) => {
  try {
    ...
    const deletedCategory = await NoteCategory.findByIdAndDelete(categoryId);
...
// Get all subjects by category ID
exports.getSubjectsByCategory = async (req, res) => {
  ...
  try {
    ...
    const category = await NoteCategory.findById(categoryId);
    if (!category) {
      return res.status(404).json({ success: false, message: "Category not found" });
    }

    // Fetch subjects related to this category
    ...
    return res.status(500).json({
      success: false,
      message: "Failed to fetch subjects for the category",
      error: error.message,
    });
  }
};